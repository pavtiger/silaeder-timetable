...
function constructTable(selector, grade) {
    ...
    for (let i = 0; i < table.length; i++) {
        ...
        for (let column = 0; column < cols.length; column++) {
            ...
            if (grade === -1) {
                row.append($('<td colspan="' + colspan[i][column] + '"/>').html(val));
            } else if (column >= grade_start[grade] && column < grade_start[grade] + headers_colspan[grade]) {
                row.append($('<td colspan="' + Math.min(colspan[i][column], grade_start[grade] + headers_colspan[grade] - column) + '"/>').html(val));
            }
        }
