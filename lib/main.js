        ...
        for (let i = 0; i < table[day].length; i++) {
            ...
            for (let column = 0; column < cols.length; column++) {
                ...
                if (cl === 'rgb(255,255,255)') {
                    ...
                    // Green
                    cl = '#4E9C81';
                } else if (cl === 'rgb(246.0000045,178.00000365,107.00000175)') {
                    // Sand yellow
                } else if (cl === 'rgb(108.99999989999999,158.00000684999998,235.00000065)') {
                    // Light blue apparently
                } else if (cl === 'rgb(255,153,0)') {
                    // Bright orange
                    cl = '#FF7400';
                } else {
                    // Unknown color
                ...
                if (grade === "-1" || in_range) {
                    row.append($('<td style="background-color:' + cl + ';' +
            ...
            if ($.inArray(k, columns) === -1) {
                ...
                if (table_columns[k] !== undefined) {
                    if (grade === "-1" || grade === table_columns[k]) {
                        header.append($('<th style="font-size:18px; background:linear-gradient(0deg, #004643, 10%, #004643); color:#FFF9E9" colspan="' + headers_colspan[table_columns[k]] + '"/>').html(table_columns[k]));
                    }
                }
            }
        }
